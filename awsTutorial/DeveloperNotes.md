...
1 ⋅⋅1	DAX (DynamoDB Accelerator) is a dedicated service designed to cache DynamoDB requests. It’ll automatically add the data in DAX if there’s a cache miss or update the data if it is updated in the table.
2 ⋅⋅1	fast in-memory performance, from single-digit milliseconds to microseconds
...
1 ⋅⋅1	1 RCU provides 4KB read per second for strong consistency model
2 ⋅⋅1	so you’ll need 6KB/4KB = 1.50, round off to the nearest one, 2 RCU
3 ⋅⋅1	540/60 = 9 items per second
4 ⋅⋅1	so total 9 * 2 = 18 RCUs.
-	Your app writes 150 items every second, each item is 4KB, how many WCU do you need?
1 ⋅⋅1	1 WCU provides 1KB write per second
2 ⋅⋅1	4KB/1KB = 4 WCU
3 ⋅⋅1	so, total 4 * 150 = 600 WCUs
-	Remember to round up to 1
1 ⋅⋅1	How many WCU is needed to support 100 writes per second of 512 bytes?
2 ⋅⋅1	512/1024 = 0.5 -> rounded up to 1, so we’ll need 1 * 100 = 100 WCU
...
1 ⋅⋅1	DEFAULT method using “AWS owned CMK”
2 ⋅⋅1	KMS method using “AWS managed CMK”
...
o	Install it on your machine, play around with it, you’ll fall in love with it, and the knowledge and understanding you gain by hands-on experiences will be incredible and unforgettable.
o	When using CLI, if the environment variables are set with access keys, they would take precedence over the IAM role.
o	Chain:
...
•	AWS SDKs:
o	https://aws.amazon.com/tools/
o	All ways of interactions boil down to an API call in the end. SDKs are the dominant/easiest/popular way to interact with AWS.
o	How to use SDK to interact with the AWS services on the cloud?
...
o	Access Keys consist of an access key ID and secret key which can be used to access AWS SDKs, REST or Query API operations.
...
o	In this case, S3 automatically throttles your requests to the bucket to protect the customer from an excessive amount of request traffic.
o	To determine which S3 objects have millions of versions, use the Amazon S3 Inventory tool which generates a report that provides a flat file list of the objects in a bucket.

•	AWS KMS:
•	https://aws.amazon.com/kms/faqs/
•	Which approaches allow customers to manage the encryption keys?
o	SSE with AWS KMS keys
o	SSE with Customer-Provided Keys
...
o	AWS_XRAY_DAEMON_ADDRESS
•	How to trace all incoming HTTP requests using X-Ray SDK?
o	add Interceptors to your code to do so.
•	How to use X-ray to debug Lambda:
o	make sure the IAM role assigned to the Lambda function has access to the X-ray service
o	to enable tracing on your Lambda function using the Lambda CLI:
...
o	one request per second & five percent of any additional request per host.
...
o	AWS-Specific Parameter Types
o	SSM Parameter Types
•	How to test a new function with migrating 5% of traffic to a new version?
o	aws lambda create-alias --name alias name --function-name function-name --routing-config AdditionalVersionWeights={“2” = 0.05}
•	How to make templates that can be used across multiple accounts and regions with the least amount of effort?
o	Use CloudFormation StackSets.
...
o	use cfn-init helper script in CloudFormation
...
o	AppSpec file in either JSON or YAML format

•	API Gateway:
o	https://docs.aws.amazon.com/apigateway/latest/developerguide/welcome.html
...
•	CloudFront:
o	https://aws.amazon.com/cloudfront/faqs/
o	when to configure your origin to add a Cache-Control or an Expires header field to each object
o	when to specify a value for Minimum TTL in CloudFront cache behaviors
o	To configure CloudFront to require HTTPS between viewers and CloudFront
	redirect HTTP to HTTPS
	HTTPS only
o	To configure CloudFront to require HTTPS between CloudFront and your custom origin:
	change the Origin Protocol Policy to HTTPS only.

•	AWS Cognito:
o	https://aws.amazon.com/cognito/faqs/
o	Blocking compromised users is Advanced Security setting
o	Understand Cognito User Pool vs Cognito Identity Pool

•	IAM:
o	https://aws.amazon.com/iam/faqs/
o	At least 4+ questions around IAM in real exams. Since every application needs authentication and authorization. You cannot go around it. :)

•	OpsWork:
o	https://aws.amazon.com/opsworks/stacks/faqs/
o	is best suited when you have multiple stacks and want to use configuration tools for the environment

•	Elastic Cache:
o	https://aws.amazon.com/elasticache/faqs/
o	Memcached does not offer high availability (replication) while Redis does on AWS ElastiCache

•	Elastic Beanstalk:
o	https://aws.amazon.com/elasticbeanstalk/faqs/
o	When to use Packer?
o	supports two options of saving configuration options: YAML or JSON, both can be included in your application’s source file in a directory named .ebextensions and deployed as part of your application code
o	How to change instance types in a prod environment with AWS Elastic Beanstalk?
o	How to move an existing dev env which consists mainly of Docker based containers to AWS cloud?
o	When you use the Elastic Beanstalk console to deploy a new application or an application version, you’ll need to upload a source bundle. Your source must meet the following requirements:
	Consist of a single ZIP file or WAR file
	Not exceed 512 MB
	Not include a parent folder
o	If you want to deploy a worker application that processes periodic background tasks, your application source bundle must also include a cron.yaml file.
o	The least time needed way to provision an application using Elastic Beanstalk with unique configuration files and software to include:
	use a custom AMI for the underlying instances

•	Lambda and Serverless:
o	https://aws.amazon.com/lambda/faqs/
o	How to make a function run on a scheduled basis?
	Use CloudWatch events
o	DynamoDBCrudPolicy will give CRUD permissions to a DynamoDB table which is tighter and inline with best security practice with the least privilege.
o	Avoid using recursive code in your Lambda function, this is clearly documented in AWS docs. Since it could lead to unintended volume of function invocations and escalated costs.

•	AWS CodePipeline:
o	https://aws.amazon.com/codepipeline/faqs/

•	AWS CodeBuild:
o	https://aws.amazon.com/codebuild/faqs/
o	How to allow an application that needs to access an RDS instance in a private subnet when using CodeBuild to build?
...
o	supports building at the following locations:
	AWS CodeCommit
	S3
	Github
	BitBucket
...
•	AWS CodeCommit:
o	https://aws.amazon.com/codecommit/faqs/
o	You can migrate to CodeCommit from other version control systems, but you’ll have to migrate to Git first.
o	You can migrate to CodeCommit in a number of ways: by cloning it, mirroring it, migrating all or just some of the branches and so on.
o	How to share repo easily and securely?

•	Kinesis:
o	https://aws.amazon.com/kinesis/
o	You cannot guarantee the order of data across multiple shards. It’s possible only within a shard.
o	Transferring data via SSL is for data in transit.
o	How to stream large sets of data directly into S3?
	Use Kinesis Data Firehose.
o	How to analyze data using standard SQL?
	Amazon Kinesis Data Analytics

•	Amazon ECS:
o	https://aws.amazon.com/ecs/faqs/
o	Where to store Docker images?
	ECR
	and
	Docker Hub
	but not store them as AMI
o	ECS has orchestration built-in.

•	Jenkins/CI/CD:
o	https://aws.amazon.com/getting-started/projects/set-up-ci-cd-pipeline/
